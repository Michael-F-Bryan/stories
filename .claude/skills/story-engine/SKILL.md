...
Story-engine is the positive dramatic-design skill for this repo. It turns character pressure into scene movement and arc shape.

It does not own canon, state, ledgers, spoilers, or lifecycle; `serial-process` does.
It does not rewrite prose; `authors-voice` does.
It is for design and structural diagnosis, not for line-level composition or canon rulings.
...
- Before planning or drafting, load `references/generation-packet.md`.
- After a draft exists, load `references/developmental-read.md` for structural evaluation.
- Keep unpublished plans in `plan/` and never paraphrase them in user-facing summaries.
...
Story-engine owns the why and the shape, not the sentence.
...
Stable character voice anchors may live in the bible.
Fast-changing wants, misreadings, and scene-local pressures belong in the packet/current state.
Avoid catchphrase mimicry and psych-label biographies; write for how the character sounds and acts under pressure.
...
If a state or spoiler fix is needed, route that to `serial-process`.

## Practical rule

Design from pressure outward:
what the character wants, what blocks them, what action they take, how the situation changes, and what that change makes the reader want next.
That is the work.
