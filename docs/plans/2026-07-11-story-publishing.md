...
**Tech Stack:** Node.js 22, pnpm 11, Eleventy 3, Nunjucks, Markdown-It, Pandoc 3, Node test runner, GitHub Actions, GitHub Pages.
...
2. Run `pnpm exec node --test test/books.test.js` and confirm failures are due to the missing publishing module.
...
4. Run the targeted test and full `pnpm test` until green.
...
6. Run targeted tests, full tests, and `pnpm build` until green.
...
4. Run `pnpm install --frozen-lockfile`, `pnpm test`, and `pnpm build` from a clean state.
